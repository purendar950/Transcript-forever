...
(() => {
  ...
  function ensureCloudSyncPanel() {
    ...
    if (!auth || !auth.parentNode || document.getElementById('cloudSyncPanel')) return;
  ...
  function loadCloudReliability() {
    ...
    script.onerror = () => {
      ...
      if (status) status.textContent = '⚠ Cloud sync unavailable';
    };
  ...
  function init() {
    ...
    // Lightweight count refresh only; no document-wide MutationObserver.
    setInterval(updateCloudSyncCount, 3000);
