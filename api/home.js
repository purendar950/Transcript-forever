...
(function(){
  ...
  var oldRenderList=window.renderList;window.renderList=function(){if(oldRenderList)oldRenderList();setTimeout(function(){var ws=words();document.querySelectorAll('#wordList .wordItem').forEach(function(box){var b=box.querySelector('b');if(!b)return;var wn=String(b.textContent||'').trim();var d=ws.find(function(x){return x&&String(x.word||'').trim()===wn});if(!d)return;var info=statusInfo(d.word),badge=box.querySelector('.wordStatus');if(!badge){badge=document.createElement('span');box.appendChild(badge)}badge.className='wordStatus '+info.cls;badge.textContent=info.label;badge.style.display='inline-flex'});ensureWordControls()},0)};
