export default async function handler(req, res) {
  ...
  try {
    const { baseUrl, apiKey, apiUser, action = 'models', model, messages, prompt, size = '1024x1024', type } = req.body || {};
    const cleanBaseUrl = String(baseUrl || '').trim();
    const cleanApiKey = String(apiKey || '').trim();
    if (!cleanBaseUrl) return res.status(400).json({ error: 'baseUrl is required' });
    if (action !== 'models' && !cleanApiKey) return res.status(400).json({ error: 'API key is required' });
    ...
    if (action === 'image' && host === 'gen.pollinations.ai') {
      ...
      for (const currentModel of models) {
        try {
          ...
          let upstream = await fetch(nativeUrl.toString(), { method: 'GET', headers: { Authorization: `Bearer ${cleanApiKey}`, Accept: 'image/*' } });
