export default async function handler(req, res) {
  ...
  try {
    ...
    if (!allowedHosts.has(url.hostname)) return res.status(400).json({ error: 'This app currently supports NaraRouter and OpenAI endpoints.' });
    let target, body;
    if (action === 'chat') {
      target = new URL('/v1/chat/completions', url).toString();
      body = JSON.stringify({ model, messages: messages || [{ role: 'user', content: 'Reply with OK only.' }], max_tokens: 1800 });
    } else target = new URL('/v1/models', url).toString();
  ...
  } catch (error) { return res.status(502).json({ error: 'AI connection failed', detail: error.message }); }
}
